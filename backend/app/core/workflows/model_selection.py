...
async def select_best_model(
    ...
    for window_idx, (train_df, test_df) in enumerate(splits, 1):
        ...
        for model_name in all_models:
            try:
                ...
                # 构建预测值字典（按日期）
                forecast_dict = {point.date: point.value for point in forecast_points}
    ...
    candidate_model_maes = {
        model: mae for model, mae in model_avg_maes.items() if model != baseline
    }

    if not candidate_model_maes or all(
        mae == float("inf") for mae in candidate_model_maes.values()
    ):
    ...
    is_better_than_baseline = (
        best_model != baseline
        and best_mae != float("inf")
        and baseline_mae != float("inf")
        and best_mae < baseline_mae
    )
