...
def generate_power_demand(
    ...
    if temperature is None:
        temperature = 22
    if pd.isna(temperature) or (
        isinstance(temperature, float) and math.isnan(temperature)
    ):
        temperature = 22
    try:
    ...
    season_factor = (
        ...
        + 0.015 * math.cos(2 * math.pi * (t - 0.5))
    )

    # --- 工作日因子 ---
    weekday_factor = 1.0 if not _is_weekend(date) else 0.97
...
class PowerDataFetcher:
        ...
        if historical_days > 92:
            ...
            if not archive_weather.empty and not recent_weather.empty:
                ...
                weather_df = pd.concat(
                    [archive_weather, recent_weather], ignore_index=True
                )
        ...
        for _, row in weather_df.iterrows():
            ...
            if start_dt <= date <= end_dt:
                try:
                    ...
                    print(
                        f"[PowerData] 警告: 生成日期 {date.date()} 的供电需求失败: {e}"
                    )
                    continue

        if not power_demands:
            raise ValueError(
                f"日期范围内没有生成供电需求数据: {start_date} ~ {end_date}"
            )
    ...
    def prepare(df: pd.DataFrame, target_column: str = "y") -> pd.DataFrame:
        ...
        if "ds" in df.columns and "y" in df.columns:
            result = df[["ds", "y"]].copy()
            result = (
                result.sort_values("ds").drop_duplicates("ds").reset_index(drop=True)
            )
            print(
                f"✅ 数据准备: {len(result)} 条, {result['ds'].min().date()} ~ {result['ds'].max().date()}"
            )
            return result
        ...
        result = (
            pd.DataFrame(
                {"ds": pd.to_datetime(df[date_col]), "y": df[value_col].astype(float)}
            )
            .sort_values("ds")
            .drop_duplicates("ds")
...
async def fetch_historical_same_period(
    city_name: str, target_start: datetime, target_end: datetime, years_back: int = 2
    ...
    for year_offset in range(1, years_back + 1):
        ...
        try:
            ...
            for _, row in hist_weather.iterrows():
                ...
                year_power.append(
                    {"ds": target_date, "y": demand, "year_offset": year_offset}
                )
            ...
            weather_copy["target_date"] = weather_copy["date"].apply(
                lambda d: target_start
                + timedelta(days=(pd.to_datetime(d).date() - hist_start.date()).days)
            )
    ...
    if all_weather_data:
        ...
        avg_weather = (
            weather_df.groupby("target_date")
